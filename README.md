...
```
                   ...
                   │  - Gmail API                 │
                   │  - WhatsApp API              │
                   │  - Sheets/CRM/Tally API      │
                   └──────────────┬───────────────┘
                                   │ Normalized data
                                   ▼
              ┌──────────────────────────────────────────┐
              │          Event & Task Router              │
              │  (Decides which agent needs to act)       │
              └──────────────┬────────────────────────────┘
                             │
       ┌─────────────────────┼──────────────────────────────┐
       ▼                     ▼                              ▼
┌──────────────┐    ┌────────────────┐              ┌─────────────────┐
│ Email Agent  │    │ Billing Agent  │              │  CRM Agent      │
│ (Replies)    │    │ (Invoices)     │              │ (Update leads)  │
└──────┬───────┘    └──────┬─────────┘              └──────┬──────────┘
       │                   │                                │
       ▼                   ▼                                ▼
  Drafted reply       Invoice created              Customer info updated
       │                   │                                │
       └───────────┬───────┴──────────────┬────────────────┘
                   ▼                      ▼
       ┌──────────────────┐     ┌─────────────────────────┐
       │ Approval/Policy  │     │ Reporting Agent          │
       │  Agent           │────▶│ (Daily/Weekly Reports)   │
       └──────┬───────────┘     └───────────────┬─────────┘
              │                                   │
     Approve / auto mode                          ▼
...
#### 🔵 **1. Connector Agents (Input Layer)**

These agents fetch data from external sources and normalize events:

- **Gmail Connector Agent**
  - Reads new emails via Gmail API
  - Extracts sender, subject, body
  - Emits: `NEW_EMAIL_RECEIVED`

- **WhatsApp Connector Agent**
  - Monitors customer messages via WhatsApp Business API
  - Emits: `NEW_MESSAGE`

- **Sheets/CRM/Tally Connector Agent**
  - Watches for updates in spreadsheets, CRM systems, and accounting software
  - Emits: `NEW_TASK`, `PAYMENT_DUE`, `STOCK_LOW`, etc.

#### 🟡 **2. Decision & Routing Agent (Brain Layer)**

- **Task Router Agent**
  - Central hub that analyzes incoming events
  - Routes tasks to appropriate specialized agents
  - Examples:
    - Email event → Email Agent
    - Invoice reminder → Billing Agent
    - Lead update → CRM Agent

#### 🟢 **3. Core Automation Agents (Work Layer)**

- **Email Agent**
  - Reads and summarizes customer emails
  - Drafts professional replies based on context
  - Suggests relevant attachments or data
  - Output: `REPLY_DRAFT`

- **WhatsApp Agent**
  - Writes short, friendly chat responses
  - Forwards important conversations to CRM
  - Schedules follow-ups
  - Output: `CHAT_REPLY_DRAFT`

- **Billing Agent**
  - Creates invoices automatically
  - Generates payment reminders
  - Drafts reminder messages for WhatsApp/email
  - Marks invoices as paid when confirmed
  - Output: `INVOICE_CREATED`, `PAYMENT_REMINDER_DRAFT`

- **CRM Agent**
  - Converts messages into leads
  - Updates lead status automatically
  - Assigns tags and priority levels
  - Output: `CRM_UPDATED`

- **Data Entry Agent**
  - Transfers information from emails to spreadsheets
  - Updates database fields
  - Cleans and validates data
  - Output: `DATA_UPDATED`

#### 🟠 **4. Long-Term Intelligence Agents (Adds Magic)**

- **Learning Agent**
  - Observes human responses and decisions
  - Learns communication tone, templates, and business rules
  - Updates internal policies over time
  - Makes the system truly "autopilot" after training period

- **Reporting Agent**
  - Compiles daily/weekly business reports
  - Tracks metrics:
    - Sales performance
    - Tasks completed
    - Pending invoices
    - Open leads
  - Sends summaries via email/WhatsApp
  - Output: `BUSINESS_REPORT`

#### 🔴 **5. Control & Safety Layer**

- **Policy/Approval Agent**
  - Reviews all agent outputs before execution
  - Decision modes:
    - Auto-approve (high confidence)
    - Request human approval (medium confidence)
    - Block (policy violation)
  - Ensures:
    - No incorrect messages sent
    - No accidental invoices
    - No spam or inappropriate content
  - **This protects your business reputation**

#### ⚪ **6. Execution Layer**

- **Execution Agent**
  - Final step before any action is taken
  - Sends emails, WhatsApp messages
  - Pushes invoices to accounting systems
  - Updates CRM records
  - Logs all actions for audit trail
  - **Nothing is sent without this agent's approval**

### Integration Layer

Autopilot plugs into your existing tools:
...
### How Agents Work Together (Example Flow)

**Scenario**: Customer sends an email inquiry

1. **Gmail Connector** receives the email → `NEW_EMAIL_RECEIVED`
2. **Task Router** analyzes event → routes to Email Agent
3. **Email Agent** drafts a professional reply
4. **Policy Agent** reviews draft for accuracy and tone
5. **Execution Agent** sends the email
6. **CRM Agent** logs the interaction in customer record
7. **Learning Agent** observes the exchange for future improvements
